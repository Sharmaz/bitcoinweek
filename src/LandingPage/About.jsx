...
function About() {
  ...
  return (
    <div className={`relative z-10 flex flex-col justify-center items-center transition-all ease-in-out duration-300 ${isVisible ? 'md:opacity-100 md:translate-y-4' : 'md:opacity-0'} md:my-32`} ref={ref}>
      ...
      <div className={`separator absolute bottom-[-50px] md:bottom-[-140px] w-screen h-16 md:h-[140px] bg-cover transition-all ease-in-out duration-300 ${isVisible ? 'md:opacity-100' : 'md:opacity-0'}`}>
      </div>
    </div>
  );
}
